...
备注：网上抄的
*/
uint8_t Edge,Edge1= 0;
uint8_t dis_fm,dis_fm1;//获取的距离数据
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
  UNUSED(htim);
	...
	if(htim->Instance == htim2.Instance)  //TIMER2
	{
		...
		if(htim->Channel == HAL_TIM_ACTIVE_CHANNEL_2)  //CHANNEL_2
		{
			if(Edge1 == 0)          //捕获上升沿
			{
				RisingTime = HAL_TIM_ReadCapturedValue(&htim2, TIM_CHANNEL_2);    //获取上升沿时间点
				__HAL_TIM_SET_CAPTUREPOLARITY(&htim2, TIM_CHANNEL_2, TIM_INPUTCHANNELPOLARITY_FALLING);   //切换捕获极性 下降沿
				HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_2);     //切换后重启中断
 
				Edge1 = 1;          	 //捕获极性标值换为下降沿
			}
			else if(Edge1 == 1)     //捕获下降沿
			{
				FallingTime = HAL_TIM_ReadCapturedValue(&htim2, TIM_CHANNEL_2);    //获取下降沿时间点
				__HAL_TIM_SET_CAPTUREPOLARITY(&htim2, TIM_CHANNEL_2, TIM_INPUTCHANNELPOLARITY_RISING);     //切换捕获极性 上升沿
				HAL_TIM_IC_Start_IT(&htim2, TIM_CHANNEL_2);   	 //切换捕获极性后重启中断
 
				HighTime = FallingTime < RisingTime ? FallingTime + 0xffff - RisingTime + 1 : FallingTime - RisingTime;
				//高电平时间=下降沿时间 - 上升沿时间(若下降沿时间<上升沿时间FallingTime + 0xffff - RisingTime + 1)
				dis_fm1 = HighTime * 17/ 100; //距离计算 HighTime*（1/100000）秒*34000厘米/2
				printf("distance = %dcm \r\n", dis_fm1);		
				Edge1 = 0;		  			 //一次采集完毕，捕获上升沿标志
			}
		} 
	}
